...
--Data Clearning: DELETE rows with 'Asia' in the location as it should be in the continent
...
---Selecting the data which we will be utilizing in Tableau for creation of visualization
...
--Infection rate in different location
SELECT location, CAST(population AS bigint) AS population, MAX(total_cases) AS HighestInfectionCount,  Max((total_cases/NULLIF(population, 0)))*100 AS PercentPopulationInfected
FROM PortfolioProject..CovidDeaths
GROUP BY location, population
ORDER BY PercentPopulationInfected DESC

--Infection rate in different location by date
